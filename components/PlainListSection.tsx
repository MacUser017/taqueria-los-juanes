    ...
    <>
      {sections.map((section) => (
        <section key={section.key} id={section.key} className="mx-auto max-w-3xl scroll-mt-20 px-4 py-10">
          ...
          {section.option && <p className="mt-2 text-sm font-semibold text-marigold">✦ {section.option}</p>}
              ...
              <div key={group.label ?? i}>
                ...
                <ul className="flex flex-wrap gap-2">
                  {group.items.map((name) => (
                    <li key={name} className="rounded-full border border-marigold/40 bg-black px-3 py-1 text-sm text-cocoa">
                      {name}
                    </li>
                  ))}
                </ul>
